...
from Planner.planner_addref import Planner
from common_utils_test14hard import *
...
log_file_path = os.path.join(log_dir, 'simulation_272.log')
...
def build_simulation(experiment, planner, scenarios, output_dir, simulation_dir, metric_dir):
    ...
    for scenario in tqdm(scenarios, desc='Running simulation'):
        logging.info(f"Processing scenario token: {scenario.token}")  # 记录每个scenario.token到日志中
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str, help='path to data',default="")
    parser.add_argument('--map_path', type=str, help='path to nuplan maps',default="")
