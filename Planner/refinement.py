...
class RefinementPlanner:
    ...
    def build_optimizer(self):
        ...
        occupancy = th.Variable(torch.empty(1, 30, MAX_LEN), name="occupancy")#修改为20
    ...
    def build_cost_function(self, objective, control_variables, ego_state, ego_pred_plan, speed_limit, occupancy, weights, vectorize=True):
        ...
        # collision avoidance
        safety_cost = th.AutoDiffCostFunction([control_variables], safety, 8, weights['hard_constraint'], #修改为6，一共有6个cost
    ...
    def plan(self, ego_state, init_plan, pred_plan, occupancy, ref_path):
        # initial plan
        ds = init_plan.clamp(min=0).to(torch.float32)
        s = pred_plan[:, 1:].clamp(min=0).to(torch.float32)
